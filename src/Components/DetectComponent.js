...
export default class DetectComponent extends Component {
  ...
  //Prevent shutdown with a security shortcut
  initShortcut = () => {
  ...
  getFaceDetectorScriptPath = () => {
    switch (remote.getGlobal("sharedObject").platform) {
      case "win32":
        //TODO: handle the shutdown in win32
        break;
      default:
  ...
  initDetection = (faceToMatch, facesDir) => {
    ...
    ls.stdout.on("data", (data) => {
      ...
      if (
        this.state.retryState < 2 &&
        !responseDetection.includes(AUTHORIZED)
      ) {
        this.setState({
    ...
    if (WebCamera.loaded) {
      WebCamera.snap((data_uri) => {
        ...
        electronFs.writeFile(completePath, imageBuffer.data, (err) => {
          ...
          this.initDetection(
            completePath,
            userAppDataPath + "/" + "FaceToFaceData/"
          );
        });
      });
        ...
        <div style={{ textAlign: "center" }}>
          <Grid container style={{ textAlign: "center" }}>
            ...
            <Grid
              ...
              style={{ textAlign: "center", fontFamily: "Roboto" }}
            >
              <h4 style={{ color: "#BDBDBD", padding: 0 }}>
                {strings.subtitle}
              </h4>
            </Grid>
            ...
            <Grid item xs={12} style={{ textAlign: "center" }}>
              {this.state.isAnalizing ? (
                <h4
                  style={{ color: "#BDBDBD", padding: 0, fontFamily: "Roboto" }}
                >
                  {strings.detectDescription}
                </h4>
                ...
                <h4
                  style={{ color: "#8BC34A", padding: 0, fontFamily: "Roboto" }}
                >
                  {strings.authorized}
                </h4>
              ) : (
                <h4 style={{ color: "red", padding: 0, fontFamily: "Roboto" }}>
                  {strings.intrusion}: {this.state.intrusionMessage}
                </h4>
              )}
            </Grid>
            <Grid
